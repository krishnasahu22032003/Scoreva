...
export default function AdminMatchLivePage() {
  ...
  return (
    <div className="px-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          <h1 className="text-3xl font-[var(--font-heading)] font-semibold tracking-tight">
            Live Commentary
          </h1>
        ...
        <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--live)]/10 border border-[var(--live)]/30">
          <Radio size={14} className="text-[var(--live)] animate-pulse" />
          <span className="text-xs font-medium text-[var(--live)]">
            LIVE STREAM
          </span>
        </div>
      </div>
      ...
      {/* FEED CONTAINER */}
      <div className="glass rounded-2xl border border-[var(--border-subtle)] p-6 max-h-[70vh] overflow-y-auto hide-scrollbar">
  ...
  {commentary.map((item) => (
    <motion.div
      ...
      className="relative mb-5 p-5 rounded-2xl bg-[var(--surface-elevated)] border border-[var(--border-subtle)] hover:border-[var(--border-strong)] transition-all"
    >
      {/* Accent Line */}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        ...
        {item.eventType && (
          <span className="text-[10px] px-3 py-1 rounded-full bg-[var(--cyan)]/15 text-[var(--cyan)] font-medium tracking-wide">
            {item.eventType}
          </span>
        )}
      </div>
      ...
      {/* META INFO */}
      <div className="flex flex-wrap items-center gap-4 text-[11px] text-[var(--muted)] mb-3">
      ...
      {item.tags && item.tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3">
          {item.tags.map((tag: string, index: number) => (
            <span
              key={index}
              className="text-[10px] px-2 py-1 rounded-md bg-[var(--surface)] border border-[var(--border-subtle)] text-[var(--secondary)]"
            >
              #{tag}
            </span>
          ))}
      ...
      {/* METADATA */}
      {item.metadata && (
        <details className="text-[11px] text-[var(--muted)] cursor-pointer">
          <summary className="hover:text-[var(--foreground)]">
            View Metadata
          </summary>
          <pre className="mt-2 p-3 rounded-lg bg-[var(--surface)] border border-[var(--border-subtle)] overflow-x-auto text-[10px]">
